...
const features = [
  ...
  { icon: <LocalShippingOutlinedIcon sx={{ fontSize: 32 }} />, text: 'Shipping(within 10-15 days)' },
  { icon: <CheckBoxOutlinedIcon sx={{ fontSize: 32 }} />, text: 'Framed' },
  { icon: <LockOutlinedIcon sx={{ fontSize: 32 }} />, text: '100% Secure Payment' },
];

return (
        <Stack sx={{borderTop: '1px solid #eee' }} mx={4} p={2}>
          <Grid container justifyContent={'space-between'} spacing={2} sx={{ textAlign: 'center' }}>
            {features.map((feature, index) => (
              <Grid key={index}> {/* xs={4} creates a 3-column layout */}
